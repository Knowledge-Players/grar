...
class PartController
{
	...
	**/
	...
	private function validateActivity(inputId: String):Void
	{
		var result = part.validate(inputId);
	...
	private function onInputEvent(eventType: InputEvent, inputId: String, mousePoint: Point):Void
	{
		...
		var rules = switch(eventType){
			case MOUSE_DOWN: part.getRulesByType(PartDisplay.CLICK, inputGroup);
		...
		for(rule in rules){
			switch(rule.value.toLowerCase()){
				...
				case "drop":
					var drop: Input = inputGroup.inputs.filter(function(i: Input)return i.id == targetId)[0];
                ...
                case "replacecontent" :
                    var output: Input = inputGroup.inputs.filter(function(i: Input)return i.id == input.values[0])[0];
