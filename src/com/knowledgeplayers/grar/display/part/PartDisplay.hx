    ...
    **/
	...
	private function setSpeaker(author:String, ?transition:String):Void
	{
		if(author != null && displays.exists(author)){
			...
			if(char != currentSpeaker){
				...
				cast(displays.get("name-" + currentSpeaker.model.ref.split("_")[0]).obj, ScrollPanel).setContent(currentSpeaker.model.getName());
			}
		}
		else
			currentSpeaker = null;
	...
	private function mustBeDisplayed(key:String):Bool
	{
		...
		if(currentTextItem != null){
			if(currentSpeaker != null && Std.is(object.obj, ScrollPanel) && key == "name-" + currentSpeaker.model.ref.split("_")[0])
				return true;
